...
pyenv virtualenv $PYTHON_VERSION basic_ml
pyenv activate basic_ml
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name basic_ml python=PYTHON_VERSION
...
Tests can be found in `basic_ml_tests` and are run with the following command:

```bash
pytest basic_ml_tests
```

As you create Dagster ops and graphs, add tests in `basic_ml_tests/` to check that your
